(function(){
    ...
    //    modalOverlay.classList.add("show"); // 모달을 화면에 표시
    //  });
 
    //  // 모달 닫기
    //  closeModalBtn.addEventListener("click", () => {
    //    modalOverlay.classList.remove("show"); // 모달을 화면에서 숨김
    //  });
    ...
    //  });
     ...
     async function prepareStudentListTable() {
        // Return a promise that resolves when the table is fully prepared
        ...
        try {
            ...
            await prepareStudentListTable(); // Now this properly awaits the Promise
        } catch (error) {
    ...
    renderStudentListTable();
        ...
        downloadSampleBtn.addEventListener('click', (e) => {
            ...
            const sampleFileUrl = 'Student_Submit_Form_Sample.xlsx'; // Path to your uploaded template
        ...
        // Add more students functionality
        const addMoreStudentsBtn = document.getElementById('add-more-students');
        const studentEntriesContainer = document.getElementById('student-entries-container');
        let studentCount = 1;
        
        addMoreStudentsBtn.addEventListener('click', () => {
            studentCount++;
            ...
            studentEntriesContainer.appendChild(newEntry);
        ...
        // Function to reindex student entries after removal
        function reindexStudentEntries() {
            const entries = studentEntriesContainer.querySelectorAll('.student-entry');
        ...
        function generateStudentId() {
            // This is a simple implementation - in a real app you'd likely get this from the server
            return 1000000 + Math.floor(Math.random() * 1000000);
        }
        
        // Function to submit student data to the server
        async function submitStudentData(studentData) {
            try {
                ...
                if (!response.ok) {
                    throw new Error('Failed to add student(s)');
                }
        ...
        submitSingleBtn.addEventListener('click', async () => {
            ...
            // Prepare data in the format matching your Google Sheet
            const studentData = {
            ...
            try {
                // In development, log the data instead of submitting
                ...
                // You might want to trigger a reload of the student list here
        ...
        // Multiple students submission
        submitMultipleBtn.addEventListener('click', async () => {
            // Get common data
            ...
            try {
                ...
                // Uncomment to actually submit when ready
                // const result = await submitStudentData(studentsData);
                // console.log('Server response:', result);
        ...
        fileUploadForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                // In development, log instead of submitting
                console.log('Uploading file:', file.name);
                
                // Uncomment to actually upload when ready
                /*
